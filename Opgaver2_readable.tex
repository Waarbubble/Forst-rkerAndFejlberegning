...
\end{enumerate}
Then replace the ideal op-amp by a \(\mu\)A741C at \SI{25}{\celsius}, supplies \SIrange{-15}{15}{V}:
...
For an unloaded voltage follower with non-ideal op-amp model, first-order gain:
\[
\frac{v_o}{v_s}\approx\frac{A_{OL}}{1+A_{OL}}.
\]
If an explicit load $R_L$ is included with series output resistance $R_o$, multiply by $R_L/(R_L+R_o)$.
...
\]
for the follower model (dominant effect of bootstrapping). With $R_{in}=\SI{1}{M\Omega}$ and $A_{OL}=10^5$, this is extremely high and far above the simple no-feedback finite-input-resistance case.
